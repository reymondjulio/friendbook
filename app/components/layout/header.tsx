...
export default function Header() {
  ...
  const navItems = userSession?.id ? navItemsAuthenticated : navItemsUnauthenticated;
      ...
      <nav className="container flex flex-wrap max-w-6xl py-4 md:py-2 sm:px-14 md:px-12 lg:px-8 justify-center md:justify-normal gap-y-4 gap-x-4 items-center">
        ...
        <div className="md:mr-12">
          <form action="">
            ...
            <input className="bg-slate-100 rounded-full focus:outline-none py-2 px-8 text-sm text-center items-stretch" type="search" placeholder="Search Friends" />
          </form>
        </div>
        ...
        <ul className="flex gap-x-4 md:gap-x-8 lg:gap-x-16">
          {navItems.map((navItem) => {
            return (
              <li key={navItem.to}>
                <Link className="text-white font-semibold text-lg" to={navItem.to}>
                  {navItem.text}
                </Link>
              </li>
            );
